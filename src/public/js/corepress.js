...
(function(){const w="https://script.google.com/macros/s/AKfycbx_5eoaSzdQR7BkWfYLIOGpoLjn_xZTb_NVRCASlu7ITSBC0pff2UunBUoG660AYriLug/exec";let a=["Entry"],e="Direct/Closed",s=!1;const gV=()=>typeof UIDManager!=='undefined'?UIDManager.getPersistentId():"Unknown",gP=()=>document.querySelector('.UID')?.innerText.trim()||"None",gC=()=>localStorage.getItem("Cntry")||"SA",gO=()=>{const n=navigator.userAgent;return/Android/i.test(n)?"Android":/iPhone|iPad|iPod/i.test(n)?"iOS":/Win/i.test(n)?"Windows":/Mac/i.test(n)?"MacOS":"Other"},gB=()=>{const n=navigator.userAgent.toLowerCase();return n.includes("edg")?"Edge":n.includes("opr")?"Opera":n.includes("chrome")?"Chrome":n.includes("firefox")?"Firefox":"Safari"},send=()=>{if(s||!a.length)return;const d=JSON.stringify({entryTime:new Date().toLocaleString('sv-SE'),visitorId:gV(),productId:gP(),action:a.join(" -> "),pageUrl:location.pathname,referrer:document.referrer||"Direct",exitDestination:e,os:gO(),browser:gB(),screenRes:`${screen.width}x${screen.height} [${gC()}]`});navigator.sendBeacon?navigator.sendBeacon(w,d):fetch(w,{method:'POST',body:d,keepalive:!0}),s=!0};document.addEventListener("click",t=>{const c=t.target.closest("a, button, .iseek-btn, .copy-button, .tab-buttons button");if(c){let l="";if(c.tagName==="A"&&c.href&&(!c.href.includes(location.hostname)||c.classList.contains("buy-button")))e=c.href,l="Exit";c.classList.contains("add-to-cart")&&(l="Cart"),c.classList.contains("copy-button")&&(l="Coupon"),c.classList.contains("iseek-btn")&&(l="Alert"),c.id==="btn-download-chart"&&(l="Download"),c.closest(".tab-buttons")&&(l="Tab:"+c.innerText.trim()),l&&a[a.length-1]!==l&&(a.push(l),s=!1)}}),window.addEventListener("pagehide",send),window.addEventListener("visibilitychange",()=>{"hidden"===document.visibilityState?send():(s=!1,a.push("Resume"))})})();
